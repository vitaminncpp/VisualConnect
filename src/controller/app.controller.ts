...
export async function login(req: Request, res: Response)
{
    ...
    {
        return res.status(500).send({error: "Internal Server Error", description: err, trace: new Error().stack});
    }
}
...
export async function getUser(req: Request, res: Response)
{
    ...
    try
    {
        ...
        UserModel.findOne({username}, (err: any, user: any) =>
        {
            if (err)
            {
                res.status(500).send({error: "Invalid username", description: err, trace: new Error().stack});
            }
            if (!user)
            {
